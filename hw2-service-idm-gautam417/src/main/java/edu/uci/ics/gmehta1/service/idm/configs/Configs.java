package edu.uci.ics.gmehta1.service.idm.configs;

import edu.uci.ics.gmehta1.service.idm.logger.ServiceLogger;
import edu.uci.ics.gmehta1.service.idm.models.ConfigsModel;
...
public class Configs {
    ...
    private final int    DEFAULT_PORT = 5360;
    ...
    public Configs(ConfigsModel cm) throws NullPointerException {
        if (cm == null) {
            ...
            if (timeout <= 0) {
                System.err.println("No session timeout found in configuration file.");
            } else {
                System.err.println("Session timeout: " + timeout + " ms");
            }
            expiration = Long.parseLong(cm.getSessionConfig().get("expiration"));
            if (expiration <= 0) {
                System.err.println("No session expiration found in configuration file.");
            } else {
                System.err.println("Session expiration: " + expiration +" ms");
            }
